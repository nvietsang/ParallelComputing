...
void calculate_dgemm(const int n, const int nMatrices,
        double *A_arr, double *B_arr, double *C_arr,
        double &computation_time, double &offload_time){
    ...
    for(int i = 0; i < nMatrices; i++){
        ...
        /* TODO */
        /* use #pragma offload target(...) to offload this block to MIC device */
        /* please do not forget the direction and size of data: matrix A, B, C */
        {
...
void create_reference(const int n, const int nMatrices,
        double *A_arr, double *B_arr, double *C_ref){
    for(int i = 0; i < nMatrices; i++){
        double *A = &A_arr[i*n*n];
        double *B = &B_arr[i*n*n];
...
#pragma omp parallel for
    ...
    printf("\033[1m%5s %12s %12s %12s %12s %12s %12s\033[0m\n", "Trials", "MIC time", "Comm. time", "Total time", "MIC perf.", "Bandwidth", "Total perf");
    ...
    for(int trial = 1; trial <= nTrials; trial++){
        ...
        printf("%5d %12.4f %12.4f %12.4f %12.1f %12.3f %12.1f %s\n", trial, computation_time,
            offload_time, (tEnd-tStart), HztoPerf/computation_time, HztoGBs/offload_time,
